...
data Union (r :: EffectRow) (rWoven :: EffectRow) a where
  Union :: !(UnionDetails r rWoven a e) -> Union r rWoven a

data UnionDetails r rWoven a e = UnionDetails
...
instance Functor (Union r rWoven) where
  fmap f (Union (UnionDetails w t)) = Union $ UnionDetails w $ fmap f t
  {-# INLINE fmap #-}


data Weaving e rAfter resultType where
  Weaving
    :: forall f e rBefore a resultType rAfter. (Functor f)
    => !(WeavingDetails f e rBefore a resultType rAfter)
    -> Weaving e rAfter resultType

data Functor f => WeavingDetails f e rBefore a resultType rAfter =
  WeavingDetails {
  ...
  , weaveDistrib :: forall x. f (Sem rBefore x) -> Sem rAfter (f x)
...
weave
    :: Functor s
    => s ()
    -> (∀ x. s (Sem rBefore x) -> Sem rAfter (s x))
    -> (∀ x. s x -> Maybe x)
    -> Union r rBefore a
    -> Union r rAfter (s a)
...
inj :: forall e r rWoven a. (Member e r) => e (Sem rWoven) a -> Union r rWoven a
inj e = injWeaving $
...
injUsing :: forall e r rWoven a.
  ElemOf e r -> e (Sem rWoven) a -> Union r rWoven a
